...
logger = logging.Logger("scraper-underarmour")
...
def run() -> dict[str, Any]:
    logger.info("=== Under Armour scraper start ===")
    ...
    def _on_batch_done(batch_products: list[dict[str, Any]], batch_num: int) -> None:
        nonlocal upsert_ok, upsert_fail
        rows = [_to_db_row(r) for r in batch_products if r.get("image_embedding") or r.get("info_embedding")]
        if not rows:
            return
        ok, fail = supa.upsert_products(rows, batch_size=cfg.BATCH_SIZE)
        upsert_ok += ok
        upsert_fail += fail
        logger.info("Incremental upsert batch %s: ok=%s fail=%s", batch_num, ok, fail)
    ...
    # Final upsert catch-all (text embeddings + any leftover rows)
    ...
    for k, v in summary.items():
        logger.info("%s: %s", k, v)
    logger.info("=== Under Armour scraper complete ===")
    return summary
