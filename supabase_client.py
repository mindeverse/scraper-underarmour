...
class SupabaseClient:
    ...
    def _sanitize_row(row: dict[str, Any]) -> dict[str, Any]:
        """Omit embedding_version and null embedding fields (avoid PGRST102/204)."""
        out = {}
        for k, v in row.items():
            if k == "embedding_version":
                continue
            if k in ("image_embedding", "back_image_embedding", "info_embedding") and v is None:
                continue
            out[k] = v
        return out
